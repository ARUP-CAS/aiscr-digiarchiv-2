        ...
        function plothover(event, pos, item) {
            ...
            if (item) {
                ...
                if (closestTrace.distance < maxDistance + 1)
                    ...
                    plot.hideTooltip();
            } else if (that.plotOptions.series.bars.show && that.tooltipOptions.bars === true) {
                ...
                $.each(plot.getData(), function (i, series) {
                    ...
                    var pointPrev = { x: series.data[xBeforeIndex][0], y: series.data[xBeforeIndex][1] },
                        ...
                        // var distToLine = Math.min(Math.abs(series.xaxis.p2c(pos.x) - series.xaxis.p2c(pointPrev.x)),
                        // Math.abs(series.xaxis.p2c(pos.x) - series.xaxis.p2c(pointNext.x)));

                        var distToLine = Math.abs(series.xaxis.p2c(pos.x) - series.xaxis.p2c(pointPrev.x));

                    if (distToLine < closestTrace.distance) {

                        var closestIndex = xBeforeIndex;
                            // var closestIndex = Math.abs(pointPrev.x - pos.x) <
                            // Math.abs(pos.x - pointNext.x) ? xBeforeIndex : xAfterIndex;
