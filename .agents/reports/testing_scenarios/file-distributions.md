...
- **Rate limiter** (`AppState.canGetFileInterval`): keyed by IP only (the id parameter only exempts `thumb`). Two branches: a concurrent request gets 429 "Downloading file still in progress", and a request inside the `requestInterval` window after the previous **successful** download gets 429 "Try in N seconds." — the interval remainder is computed in milliseconds, so sub-second windows block. Since the #1117 fix (`d944b598`) both branches convert milliseconds to seconds with a ceiling (never `0`, no raw milliseconds), and since its follow-up `51fc0e6d` the conversion is cast to `int` — `(int)Math.ceil(ms*.001)` — so the header and the message render an **integer** (`Retry-After: 1`, "Try in 1 seconds."), a valid RFC 9110 `delay-seconds` value. With `requestInterval` = 500 ms the enforced window is 500 ms (the documented "≥ 1 s between requests" is the API doc's contract; the enforced value follows the deployment's config) and both branches report `1`. The reader's `full` action applies the same limiter but **only reads** its state — it never calls `writeGetFileStarted`/`writeGetFileFinished`, so its interval branch fires only when a File API download succeeded within the window, and its in-progress branch returns 429 **without** a `Retry-After` header by design (`web/docs/image-servlet.md`). Applies identically to original files, distributions, and paradata.
...
## Current verification (2026-09-24, `digiarchiv-test`, build `v4.0.3-234-gfd7c9c12` of 2026-09-24 — clean build, base commit `fd7c9c12` contains the #1117 follow-up `51fc0e6d` that casts the conversion to `int`; anonymous session)

Regression pass on file-distributions-D08 (the decimal `Retry-After` rendering), requested after the maintainer's follow-up fix. Candidates resolved by the discovery recipes, as in the 2026-09-23 pass: [C-TX-192700656](https://digiarchiv-test.aiscr.cz/id/C-TX-192700656) (`soubor_distri` facet on `entity=dokument`; distributions `orig`, `atr/alto-xml`, `atr/stats-csv`, soubor id `soub-649332`) and [C-3D-202500002](https://digiarchiv-test.aiscr.cz/id/C-3D-202500002) (4.25 MB public file, recipe 2) for the in-progress window.

### Verified behaviour matrix

| Capability | Result |
| --- | --- |
| Interval branch, File API | 429 with `Retry-After: 1` and body "Try in 1 seconds." — an integer, valid RFC 9110 `delay-seconds`; value ≥ 1, message seconds equal to the header (D08 fixed) |
| In-progress branch, File API | 429 with `Retry-After: 1` and body "Downloading file still in progress. Try later." — integer (D08 fixed; was `1.0` on 2026-09-23, `500` before #1117) |
| Interval branch, reader `/api/img/full` | 429 with `Retry-After: 1` and body "Try in 1 seconds." — integer, message equals header (D08 fixed; the pair needed one retry — see recipe 6) |
| In-progress branch, reader `/api/img/full` | not examined this run — verified headerless by design on 2026-09-23, outside the three #1117 fix sites |
| Distribution download via File API `/id/…/file/{uuid}/{dist}` | works — 200 with `Content-Type`/`Content-Disposition` from the `distribuce` entry: `atr/stats-csv` → `text/plain; charset=UTF-8`, `distribution_2.txt`, 3 B; `C3D202500002.jpg` → `image/jpeg`, served chunked |
| Reader `/api/img/full` serving | works — 200 for `id=soub-649332&dist=atr/stats-csv` with `Content-Type: text/plain; charset=UTF-8`, body identical to the File API form |
...
### Defect walk (from the 2026-09-23 verification)

- file-distributions-D01 — **still fixed** (this run): distributions served on the documented File API under its access rules and rate limiter (distribution downloads verified 200 with correct type/disposition; the 429s above fired on distribution paths).
- file-distributions-D02 — **still fixed** (this run): the File API rate limiter covers distribution downloads.
...
- file-distributions-D05 — **still fixed** (this run): `soubor_distri` facet present for dokument (knihovna_3d and samostatny_nalez not re-queried this run; unchanged since 2026-09-23).
...
- file-distributions-D08 — **fixed** (this run): the follow-up `51fc0e6d` casts the seconds conversion to `int` — `(int)Math.ceil(ms*.001)` — and all three sites now render an integer. Live-verified on both File API branches and the reader's interval branch: `Retry-After: 1`, "Try in 1 seconds." — the #1117 acceptance expectation ("Po opravě očekáváno `Retry-After: 1`") is now met literally.

### Known defects

- None are open on this scenario.

### Observations (not acceptance defects)

- The `permissions` scenario's durable limiter note still records the pre-#1117 values (`Retry-After: 0` interval, `500` concurrent) with their live-verified dates; its next run should repoint it at this scenario's limiter facts rather than restating them.
- The enforced window equals `requestInterval` (500 ms in the deployment config), not the documented "≥ 1 s" — unchanged, pre-existing.
- The deployed build is clean (`v4.0.3-234-gfd7c9c12`, no `-dirty` suffix), unlike the 2026-09-23 local-build practice.

### Corrections made during this run

- None — the 2026-09-23 conclusion (D08 open) was resolved by the maintainer's fix, not by new evidence contradicting it.

### Maintainer-assisted checks

- None — the retest surface is fully anonymous-probeable; no check required a role, credentials, or a browser.
...
| 2026-09-24 | `digiarchiv-test` (build `v4.0.3-234-gfd7c9c12`, clean, contains follow-up `51fc0e6d`) | D08 regression pass: the `int` cast makes all three sites render an integer — `Retry-After: 1`, "Try in 1 seconds." — live-verified on both File API branches and the reader's interval branch; D01–D07 re-verified fresh (except the reader's in-progress branch, unchanged by design); no open defects remain. |
