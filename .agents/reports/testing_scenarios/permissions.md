...
The deployed `SamostatnyNalez.filterOAI` follows the table's A/B rows on the 2026-09-15 build (the `pristupnost <= userPr` divergence is gone — permissions-D06 fixed); the C-branch creator-organisation clauses work on the entities path and — since the 2026-09-17 deploy — on OAI as well (both org arms; see D08), while the B-owner clause remains dead on OAI (permissions-D08 residual).
...
## Current verification (2026-09-17 regression pass on the #370 follow-up deploy; test-side — production probes declined)
...
**Provenance note:** the 2026-09-17 pass re-verified the D06/D08/D09 open cells with fresh role sessions (B/C/D) and anonymous/OAI probes on the new test deploy, the D07/D10/D11/D12/D13/D14 holdouts on test, and the 404/410/401 distinction; production was **not examined** (the operator declined production probes for this run — production cells carry their last observed state, 2026-09-15); cells carrying only earlier dates were observed on a previous deploy of the same fix-wave family and were not re-probed this pass.

**Correction recorded this pass:** the run's first reading of the unified file gate was "D09 fixed — the per-role rules now apply uniformly regardless of authentication"; fetching the published File API rule table (<https://arup-cas.github.io/aiscr-api-home/file-api/>) then showed the SN A/B rows carry no pristupnost conditions, so the unified-but-stricter gate is a deviation, not a fix — the operator confirmed 2026-09-17 it is a defect, not desired behaviour. The original conclusion is preserved here per the corrections policy.

### 2026-09-17 pass summary — defect outcomes

| Defect | Outcome on test | Production posture |
| --- | --- | --- |
| D06 | **still fixed**: anonymous and B receive public stav=4 SNs on landing (re-verified 2026-09-17: `C-202600010-N00085` anon 200, B 200, chranene masked) | not examined 2026-09-17 — fix wave not deployed there as of 2026-09-15 |
| D07 | **still fixed**: restricted VBs stay countable (8 elevated) but the anonymous search response carries no geometry fields (re-verified 2026-09-17) | **not examined 2026-09-17** — last observed 2026-09-15: open, deferred by operator to the upcoming production release (`entity=vyskovy_bod&q=-stav:3` → numFound 11,530 restricted-children docs; first doc served full unsuffixed geometry) |
| D08 | **narrowed further — still open**: the C creator-organisation clauses (both arms, incl. the projekt arm) now work on OAI (2026-09-17: `C-202009779-N00022` and `C-202211308-N00230` as C — record served, chranene masked per pr=D); the **B-owner clause is still dead on OAI** (`C-202600009-N00014` as B — 403 body; landing 200 for the owner) | — |
| D09 | **open — confirmed defect** (operator 2026-09-17: not desired behaviour; the published File API rule table is authoritative): the file gate now applies its per-role rows uniformly regardless of authentication (the anon/B inconsistency is gone), but the deployed SN A/B rows carry pristupnost conditions the published table does not state — anonymous and B both get 403 on stav=4 SN files with elevated pristupnost (`C-202600010-N00085`: anon 403, B 403, C 200, D 200; 3,397 test records in this class), where the published table promises `stav=4` alone suffices for A/B; the SN01 owner clause works (B 200 on own stav=1 SN file); fix direction: drop the pristupnost conditions from the SN A/B rows | — |
| D10 | **still fixed**: variant matrix conforms (orig 403, `/thumb` 200, `/thumb/page/1` 403, `/thumb-large` 403, `/paradata` 403 — anonymous; re-probed 2026-09-17) | not examined 2026-09-17 — endpoint absent there as of 2026-09-15 (404) |
| D11 | **still fixed**: komponenta search serves no chranene block to anonymous (`C-202600010-N00085-K001`; re-verified 2026-09-17) | not examined 2026-09-17 — production indexed komponenta docs (250,496) but its `entity=komponenta` query path errored as of 2026-09-15 (old searcher); the komponenta scenario owns the record-level surface |
| D12 | **still fixed**: 38 restricted projekts searchable, no chranene block in anonymous docs (re-verified 2026-09-17) | not examined 2026-09-17 — last observed 2026-09-15: fixed (`M-202400606` masked) |
| D13 | read actions (`GET_ID`, `REQUEST`) removed — 500 no-enum from inside the gate-passed branch (re-observed 2026-09-17); **closed — solved in current state** (operator 2026-09-17: the network-trusting gate posture and unprobed mutating actions accepted as-is) | not examined 2026-09-17 — last observed 2026-09-15: fixed by hotfix (actions removed, 500 no-enum); closed on the same operator decision |
| D14 | **still fixed**: `/pdf` 401 anonymous on a restricted dokument (re-verified 2026-09-17) | **not examined 2026-09-17** — last observed 2026-09-15: open, live leak, deferred by operator to the upcoming production release (`/pdf` served real page JPEGs of restricted dokumenty to anonymous clients — warm cache; two samples: 115,946 B and 55,523 B — while the gated `/thumb/page/1` returned 403) |

**Issue #370 checklist mapping (2026-09-17):**

1. *Non-archived records visible after login (landing only)* — ✓ re-verified 2026-09-17: `C-202204147A` anon 401 / B 200; `C-TX-193001369` B 200 (C/D 200 verified 2026-09-11).
2. *Same conditions as the API (#237)* — ✓ at record level on the test build (SN record rows conform; re-verified 2026-09-17 via landing/OAI probes); remaining divergences are D08 (OAI B-owner clause) and D09 (file-gate SN A/B rows vs the published File API table) — D09 now also denies anonymous where the published table promises access.
3. *404/410/401/403 distinction* — ✓ re-verified anonymous 2026-09-17: fabricated ident 404; deleted SN `C-202204159-N00003` 410; unarchived akce/dokument 401; (403 cells carried from the fix-wave passes).
4. *Correct return codes* — ✓ on the test build across surfaces (re-verified 2026-09-17: landing codes, `/pdf` 401, file-gate 403s); production residuals unchanged since 2026-09-15 (not examined this run): `/pdf` 200 with real content instead of 401 (D14), VB geometry served in search (D07).
...
| akce unarchived (`C-202204147A`) | 401 ✓ | 200 ✓ (2026-09-17 — re-verified) | 200 ✓ (2026-09-11) | 200 ✓ |
| AZ pr=B (`M-200500013A`) | 200 ✓ (masked) | 200 ✓ (full) | 200 ✓ (full) | 200 ✓ |
| dokument stav=3 pr=C | 200 ✓ | 200 ✓ | 200 ✓ | 200 ✓ |
| dokument unarchived (`C-TX-193001369`) | 401 ✓ | 200 ✓ (2026-09-17 — re-verified) | 200 ✓ | 200 ✓ |
...
| SN pr=C stav=4 (`C-202600010-N00085`) | 200 ✓ (masked; 2026-09-17 — re-verified) | 200 ✓ (masked; 2026-09-17 — re-verified) | 200 ✓ (2026-09-11) | 200 ✓ |
| SN pr=C stav=1 B-owned (`C-202600009-N00014`) | 401 ✓ | 200 ✓ (owner; 2026-09-17 — re-verified) | 200 ✓ (2026-09-11) | 200 ✓ (2026-09-11) |
...
| deleted SN (`C-202204159-N00003`) | 410 ✓ (2026-09-17 — re-verified) | 410 ✓ | 410 ✓ | 410 ✓ |
| fabricated ident | 404 ✓ (2026-09-17 — re-verified) | 404 ✓ | 404 ✓ | 404 ✓ |
...
| SN pr=C stav=1 B-owned | 403 body ✓ | 403 body ✗D08 (owner clause dead on OAI; 2026-09-17 — re-verified) | 403 body ✓ (org mismatch — rule-conformant; 2026-09-11) | 200 ✓ (full) |
...
| SN pr=D stav=1 org-match (`C-202009779-N00022`, both arms match) | 403 body ✓ | — | 200 ✓ (chranene masked per pr=D; 2026-09-17 — org clause works on OAI now) | 200 ✓ (full; 2026-09-15) |
| SN pr=D stav=1 projekt-org match (`C-202211308-N00230`, predano arm mismatches) | 403 body ✓ | — | 200 ✓ (chranene masked per pr=D; 2026-09-17 — projekt-arm org clause works on OAI now) | 200 ✓ (full; 2026-09-15) |
...
| └ `/thumb` | 200 ✓ (documented ungated; 2026-09-17 — re-verified) | — | — | — |
| └ `/thumb/page/1` | 403 ✓ (2026-09-17 — re-verified) | — | — | — |
| └ `/thumb-large` | 403 ✓ — **still fixed 2026-09-17** (D10) | — | — | — |
| └ `/paradata` | 403 ✓ (2026-09-17 — re-verified) | — | — | — |
| SN pr=C stav=4 (`C-202600010-N00085`) — orig | 403 ✗D09 (2026-09-17 — changed shape: uniform gate, stricter than the documented A row `stav=4`) | 403 ✗D09 (2026-09-17 — matches the deployed B row, stricter than the documented `stav=4 OR own`) | 200 ✓ (2026-09-17) | 200 ✓ (2026-09-17) |
...
| `/pdf?id=<restricted dokument soubor id>&page=1` | ✓ 401 — **still fixed 2026-09-17** (D14: `ImageAccess.isAllowed(…, true)` gate; re-verified on `soub-340129`) | — | — | — |
| `/pdf?id=<SN soubor id>&page=1` | ✓ gate passes per #85; cold cache serves the 7,525 B placeholder only — no content leak | — | — | — |
| production `/pdf?id=<restricted dokument>&page=1` | not examined 2026-09-17 — last observed 2026-09-15: ✗ 200 with **real page JPEG content** (115,946 B for `soub-340129`, 55,523 B for `soub-847150` — both restricted dokumenty; the page cache had warmed, D14 live leak) while the gated `/thumb/page/1` returned 403 | — | — | — |
...
| D07 | Medium-High | search API (vyskovy_bod) | **fixed on test** (2026-09-17: re-verified — 8 elevated restricted-children docs, no geometry fields); **open on production — deferred** (operator 2026-09-15: resolved by the upcoming production release; not examined 2026-09-17) | restricted VB geometry anonymously searchable |
| D08 | High | OAI-PMH (SN) | **open — narrowed to the owner clause** (2026-09-17: the C creator-organisation clauses work on OAI — both arms and the projekt arm; the B-owner clause is still dead on OAI while the landing/entities path serves it) | SN OAI owner/organisation clauses read fields the oai doc does not resolve |
| D09 | Medium | file API | **open — confirmed defect** (operator 2026-09-17: not desired behaviour; the published File API rule table is authoritative — anon/B 403 on stav=4 SN files with elevated pristupnost, 3,397 test records) | file-gate SN A/B rows over-block: pristupnost conditions the documented rule table does not state |
...
| D12 | High | search API (projekt) | **fixed on test and production** (test re-verified 2026-09-17; production last verified 2026-09-15 — `M-202400606` masked; not examined 2026-09-17) | anonymous search served full `projekt_chranene_udaje` for restricted projekts |
...
| D14 | High | `/pdf` | **fixed on test** (2026-09-17: 401 anonymous re-verified); **open on production — live leak, deferred** (operator 2026-09-15: resolved by the upcoming production release; not examined 2026-09-17) | PDF page JPEGs served with no permission gate |
