...
| --- | --- | --- | --- |
| Test | `https://digiarchiv-test.aiscr.cz/id/<ident_cely>` | `https://api-test.aiscr.cz/2.2/oai` | has #370 plus the 2026-09 fix wave (legacy `/api/search/id` removed; `/pdf` gated via `ImageAccess`; D06 fixed — A/B branches follow #237's `stav=4` rule). Remaining divergences from the published rules: D08 (OAI owner clause), D09 (file-gate SN rows), D12 (restricted stav=6 projekts, reopened 2026-09-23), D15 (projekt file gate, 2026-09-23), D17 (SN organisation clauses on the entities path, 2026-09-23). Build identity (2026-09-23): the served bundle embeds `v4.0.3-225-g45b9e5b1` (clean) while the backend behaves as the later `749f7ec0` — the footer hash does not identify the backend deploy; establish the backend by behaviour |
| Production | `https://digiarchiv.aiscr.cz/id/<ident_cely>` | `https://api.aiscr.cz/2.2/oai` | re-probed anonymously 2026-09-23 (operator-approved): still the pre-fix-wave build — D07 (restricted-children VB geometry) and D14 (`/pdf`) remain open there; both are resolved by the upcoming production release of the fix wave. The partial hotfix removed the FedoraServlet read actions (`GET_ID`/`REQUEST`), closing the D13 raw-read path. The SN and projekt file gates follow the published rule table (D09 and D15 absent), with no D12 stav=6 exposure; D17 not examined. `/thumb-large` absent (404) |
...
- **Rate limiting:** File API and `/img/full` requests are tracked per IP + file; keep **≥ 1 s between requests**; a duplicate inside the window returns **429** with `Retry-After: 0` (interval branch, live-verified 2026-09-10 and re-observed 2026-09-11); the concurrent-in-progress branch returns **429 with `Retry-After: 500`** (live-verified 2026-09-23 — the value reads as milliseconds in a header defined in seconds). The limiter keys on client IP, so proxy buffering or egress-IP rotation can defeat observation from outside.
...
Anchors from the 2026-09-10/11/23 passes on `digiarchiv-test`. **Drift rule:** records change state; re-verify each record's current state (OAI or search) before using it as a probe, and re-derive replacements via the recipes when it has drifted. File UUIDs come from the record's `soubor` elements and are **environment-specific** (the same record can bind different file UUIDs on test and production — re-read the soubor path per environment).

| Ident | Type | Properties that make it a probe | Verified on |
| --- | --- | --- | --- |
| `C-202009490A` | akce | pristupnost C, archived (stav=3), restricted element case; carries DJ `-D01` and komponenta `-K001` | landing, OAI, search, handle, file (children), element ladder (last checked 2026-09-11) |
| `M-200500013A` | archeologicky_zaznam | pristupnost B — element ladder one step up | landing, element ladder (last checked 2026-09-10) |
| `C-202204147A` | akce | unarchived → anonymous 401 case | landing (last checked 2026-09-23) |
| `C-TX-193001369` | dokument | unarchived → anonymous 401; carries dokument_cast `-D001` | landing, children (last checked 2026-09-23) |
| `M-TX-202300441` | dokument | pr=C, stav=3; file `3a0f7078-e26f-404d-aaa9-2b5abbb5e3d2` (real PNG; soubor id `soub-607938` for `/img` probes — the uuid binding drifted to the `"neni"` placeholder mid-pass 2026-09-10, see file-grid notes) | file API all endpoints, `/img` (last checked 2026-09-17) |
| `M-TX-202100125` | dokument | pr=C, stav=3 — restricted file sample; test file `3a0f7078-e26f-404d-aaa9-2b5abbb5e3d2`, production file `fe377f16-1da6-4912-b8d4-6f3095324879` (soub-340129 — the soubor id is the same in both environments) | file API, `/pdf`, `/thumb-large` (test last checked 2026-09-17; production `/pdf` re-probed 2026-09-23 — D14 open there) |
| `ADB-PRAH71-000861` | adb | restricted (stav≠3), pr=C; carries VB `-V0001` | landing, OAI, search, handle, children (last checked 2026-09-10) |
| `P-2213-100119` | pian | pr=C (HES-000867) — chranene geometry probe; exact coordinates in the raw Fedora XML | search, geometrie, `/exp` mapa, `/fedora` (D13) (last checked 2026-09-10) |
| `C-202402033` | projekt | **deleted (2026-09-23)** — tombstone (410) case: landing and handle 410 for all roles, OAI `status="deleted"` | search (last checked 2026-09-23) |
| `C-201122587` | projekt | stav=6 with oznamovatel (PII masking case) | OAI oznamovatel (last checked 2026-09-10) |
| `C-202600010` | projekt | stav=3, organizace ORG-000210 (other than the test accounts'); file `342b7b35-fdc2-4fc7-b2df-bb3c0eecc524` (projekt file row) | file API (last checked 2026-09-23) |
| `C-202101848` | projekt | stav=6, organizace ORG-000030; PDF file `06fbe1a2-62cc-4d5b-9674-abbd8c9754a4` — D15 A/B probe | file API (last checked 2026-09-23) |
| `M-200500013` | projekt | pr=B, stav=6 — D12 stav=6 sample | search, handle (last checked 2026-09-23) |
| `C-202600009-N00014` | samostatny_nalez | pr=C, stav=1, **created by the B test account** (owner clause); file `c74d3136-3cab-468a-8405-3f79b89ab555`; carries komponenta `-K001` | landing, OAI, file, children, backend (last checked 2026-09-23) |
| `C-202600010-N00085` | samostatny_nalez | pr=C, stav=4 (public-archived SN); files `cafc9a90-3b8b-40f8-ace6-5791b0b6ffd2` (soub-653685), `e5115f1a-72ce-4eaa-a2d9-f52544aaa370` (soub-653686) | landing, OAI, handle, file, backend (last checked 2026-09-23) |
| `C-202500044-N00001` | samostatny_nalez | pr=A, stav=4 — the deployed file-gate A-row sample (`pristupnost=A AND stav=4` passes); file `04d822bb-2706-430e-9ef2-720f8dcb5b8a` | file API (last checked 2026-09-17) |
| `M-202301371-N00006` | samostatny_nalez | pr=D, stav=4; projekt, `predano_organizace` and SN01 author all ORG-000066; file `0449f273-2436-457e-b118-c1067062eb58` — D09 C-row probe | file API (last checked 2026-09-23) |
| `C-202009779-N00022` | samostatny_nalez | pr=D, stav=1, maintainer-provided (2026-09-15) creator-org clause probe — both arms match: `predano_organizace` ORG-000091, projekt `C-202009779` org ORG-000091; created SN01 by U-001975 | landing, handle, OAI (last checked 2026-09-23) |
| `C-202211308-N00230` | samostatny_nalez | pr=D, stav=1, maintainer-provided (2026-09-15) creator-org clause probe — projekt arm only: `predano_organizace` ORG-000099 (Archaia Brno, mismatch), projekt `C-202211308` org ORG-000091 (match); created SN01 by U-004219 | landing, handle, OAI (last checked 2026-09-23) |
| `C-202009779-N00031` | samostatny_nalez | pr=D, stav=1 — org-match D17 probe: both its organisations are ORG-000091 | landing, handle (last checked 2026-09-23) |
| `U-004495` | uzivatel | the B test account — own-record clause | OAI (last checked 2026-09-15) |
| `U-004496` | uzivatel | the C test account (org ORG-000091) — counterpart for the creator-org clause probes | session/islogged, landing, OAI (last checked 2026-09-15) |
| `M-202500301` | projekt | production stav=6 projekt with a file — D15 production control | production file API (last checked 2026-09-23) |
| `M-202600319-N00016` | samostatny_nalez | production pr=D, stav=4 find with a file — D09 production control | production file API (last checked 2026-09-23) |
...
| D07 | **still fixed** — 8 restricted VBs, no geometry fields anonymously (2026-09-23) | **open — resolved by the upcoming production release** (2026-09-23: 11,533 restricted-children VB docs, geometry served) |
...
| D14 | **still fixed** — `/pdf` 401 anonymously (2026-09-23) | **open — live leak, resolved by the upcoming production release** (2026-09-23 re-probed) |
...
| D17 | **new — High** (re-check 2026-09-23): C receives 403 on own-organisation SN finds on landing and handle (`C-202009779-N00022`, `C-202211308-N00230` — both C 200 on 2026-09-15; `C-202009779-N00031` likewise) while OAI serves them; D 200 on all three | not examined |
...
2. *Same conditions as the API (#237)* — ✓ at record level with open deviations: D08 (OAI B-owner clause), D09 (file-gate SN rows), D12 (restricted stav=6 projekt blocks on search/handle), D15 (projekt file gate), D17 (SN organisation clauses on the entities path).
...
5. *Own-organisation finds (C)* — ✗ D17 (re-check 2026-09-23): C receives 403 on own-organisation SN finds on landing and handle while OAI serves them; the item was never ticked.

Checklist audit (2026-09-23 re-check): the older ticked items hold — child records gated anonymously and served to D, `/map?mapa=true` variants anonymous 401 / D 200, logged-in 200 pages serving the record's data, the unarchived dokument out of anonymous search, and the `N00053` file opening for B/C/D with anonymous 403 (D09).
...
| SN pr=D stav=1 org-match (`C-202009779-N00022`) | 401 ✓ (2026-09-15) | — | 403 ✗D17 (2026-09-23; was 200 ✓ 2026-09-15) | 200 ✓ |
| SN pr=D stav=1 projekt-org match (`C-202211308-N00230`) | 401 ✓ (2026-09-15) | — | 403 ✗D17 (2026-09-23; was 200 ✓ 2026-09-15) | 200 ✓ |
| SN pr=D stav=1 org-match (`C-202009779-N00031`) | — | — | 403 ✗D17 (2026-09-23) | 200 ✓ (2026-09-23) |
...
| SN pr=D stav=1 org-match (`C-202009779-N00022`, `C-202211308-N00230`, `C-202009779-N00031`) | — | — | 403 ✗D17 (2026-09-23; C 200 on 2026-09-15) | 200 ✓ (2026-09-23) |
...
| rate limiter | 429 `Retry-After: 0` ✓ (interval branch; re-observed 2026-09-11 on a burst, then clean 403 on the isolated re-probe); 429 `Retry-After: 500` ✓ (concurrent branch, 2026-09-23 — the value reads as milliseconds in a header defined in seconds) | — | — | — |

Notes: Basic auth accepted on the file path but **not** on the landing `checkId` path; landing-path requests under Basic auth are treated as anonymous; concurrent branch live-verified 2026-09-23 (`Retry-After: 500`). Test-data drift: the `M-TX-202300441` file uuid's `soubor_filepath` drifted to the `"neni"` placeholder mid-pass 2026-09-10 (recheck returns 404 for every role, including D) — the anon/B/C cells above were verified before the drift; the D cell is untestable on this record until the file binding is restored. The `/pdf` endpoint resolves by **soubor id** on the 2026-09-11 build (see surface inventory). The #693 distribution-gate pattern lives in the [`file-distributions`](file-distributions.md) scenario.
...
| `/pdf?id=<restricted dokument soubor id>&page=1` | ✓ 401 — **still fixed 2026-09-23** (D14: `ImageAccess.isAllowed(…, true)` gate) | — | — | — |
| `/pdf?id=<SN soubor id>&page=1` | ✓ gate passes per #85; cold cache serves the 7,525 B placeholder only — no content leak | — | — | — |
| production `/pdf?id=<restricted dokument>&page=1` | ✗D14 200 — real page content of a restricted dokument (2026-09-23 — live leak persists; resolved by the upcoming production release) | — | — | — |
...
| D04 | minor | file API rate limiter | **closed** (operator decision 2026-09-15: no further live verification; any reported issue is handled reactively) | both limiter branches live-verified (interval 2026-09-10; concurrent 2026-09-23 — `Retry-After: 500`) |
...
| D07 | Medium-High | search API (vyskovy_bod) | **fixed on test** (2026-09-23: re-verified); **open on production — resolved by the upcoming production release** (2026-09-23: 11,533 restricted-children docs, geometry served) | restricted VB geometry anonymously searchable |
...
| D14 | High | `/pdf` | **fixed on test** (2026-09-23: 401 anonymous); **open on production — live leak, resolved by the upcoming production release** (2026-09-23 re-probed) | PDF page JPEGs served with no permission gate |
...
| D17 | High | landing pages + handle API (SN) | **open** (2026-09-23: C 403 on own-organisation finds on landing/handle; OAI serves them; production not examined) | the SN organisation clauses regressed on the entities path |

- **D07 (Medium-High; fixed on the test build, open on production — deferred 2026-09-15 by operator decision: the upcoming production release deploys the fix):** on test, the anonymous search response for restricted VBs serves no geometry fields (8 restricted VB docs indexed, countable, no `vyskovy_bod_geom_wkt`/`geom_gml`). On production, `entity=vyskovy_bod&q=-stav:3` still returns restricted-children docs serving the full unsuffixed geometry (re-probed 2026-09-23; resolved by the upcoming production release). Fix direction unchanged: parent-derived `searchable` for VB docs and/or pristupnost-suffixed geometry fields.
...
- **D14 (High; fixed on test — re-verified 2026-09-23; open on production — live leak, deferred 2026-09-15 by operator decision: the upcoming production release deploys the gate):** the test build gates `/pdf` through `ImageAccess.isAllowed(request, true)`, keyed on the **soubor id**: a restricted dokument's page returns 401 to anonymous (re-probed 2026-09-23), and an SN soubor id passes the gate per #85 with the cold cache serving only the placeholder. Production still runs the ungated build **and the page cache has warmed**: `/pdf` serves real page JPEGs of restricted dokumenty to anonymous clients (re-probed 2026-09-23) — restricted document content is directly downloadable, not merely a status-code defect. The production release will deploy the gated build; a page-cache flush there is still advisable.
...
- **D17 (High, open — 2026-09-23):** the SN record-level organisation clauses regressed on the entities path: C (U-004496, ORG-000091) receives 403 on the landing page and the handle API for own-organisation finds — `C-202009779-N00022` and `C-202211308-N00230` (both C 200 on 2026-09-15) and `C-202009779-N00031` (both its organisations ORG-000091) all give C 403 now, while OAI serves the records (masked per pr=D) and D receives 200 on all three. This is the #370 own-finds checklist item that was never ticked. Fix direction: restore the C-branch organisation clauses on the entities path (the OAI side works). Production not examined.
...
| 2026-09-23 | 12 — #370 regression re-verification | fresh B/C/D sessions and anonymous probes on test, anonymous production probes (operator-approved): **D15 found and confirmed** (projekt file gate on the record-level rule — anon/B stav=6 files, C other-organisation files; operator: defect; production absent); **D12 reopened** (restricted stav=6 projekts unsuffixed on search/handle; production absent); **D09 extended** to the C row; **D16** reduced to the author's-organisation arm (not examinable); D08 still open; D06/D07/D10/D11/D13/D14 hold on test; production D07 and D14 still open; registry drift recorded (deleted-SN anchor live again, `C-202402033` now the 410 case); backend identity differs from the bundle hash. Post-pass re-check: **D17 found** (C 403 on own-organisation SN finds on landing/handle — org-match probes that held on 2026-09-15; OAI serves them); **concurrent 429 live-verified** (`Retry-After: 500` — reads as milliseconds in a seconds-defined header); #370 checklist audit: the older ticked items hold, the own-finds item is D17. Cleanup: superseded 2026-09-17 correction dropped, #693 pattern moved to the file-distributions scenario pointer, drifted/production-sample anchors and fixed/closed-defect long notes removed, production-leak specifics reduced to status (git history preserves them) |
