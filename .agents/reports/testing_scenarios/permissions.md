...
| --- | --- | --- | --- |
| Test | `https://digiarchiv-test.aiscr.cz/id/<ident_cely>` | `https://api-test.aiscr.cz/2.2/oai` | has #370 plus the 2026-09 fix wave; the 2026-09-15 deploy completes the D06 fix (A/B branches now follow #237's `stav=4` rule — all roles conform on public SNs) and removes the legacy `/api/search/id` action (500 no-enum); `/pdf` gated via `ImageAccess`; remaining divergences: D08 (OAI owner clause), D09 (file-gate B branch) |
| Production | `https://digiarchiv.aiscr.cz/id/<ident_cely>` | `https://api.aiscr.cz/2.2/oai` | partial hotfix deployed (2026-09-15 observed): the FedoraServlet read actions `GET_ID`/`REQUEST` are removed (500 no-enum) — the D13 raw-read leak is closed; the rest of the fix wave is **not** deployed there: `/pdf` ungated **and serving real page JPEGs of restricted dokumenty to anonymous clients** (warm cache — permissions-D14 live leak), VB geometry unsuffixed (D07), `/thumb-large` endpoint absent (404) |
...
| `M-TX-202100125` | dokument | pr=C, stav=3 — restricted file sample; test file `3a0f7078-e26f-404d-aaa9-2b5abbb5e3d2`, production file `fe377f16-1da6-4912-b8d4-6f3095324879` (soub-340129 — the soubor id is the same in both environments) | file API, `/pdf`, `/thumb-large` (2026-09-15: test variant matrix + `/pdf` 401; production `/pdf` 200 with a real 115,946 B page JPEG — D14 live leak) |
...
| `C-202600009-N00014` | samostatny_nalez | pr=C, stav=1, **created by the B test account** (owner clause); file `c74d3136-3cab-468a-8405-3f79b89ab555`; carries komponenta `-K001` | landing, OAI, file, children, backend (2026-09-15: B-owner landing 200, OAI 403 body ✗D08 — clause still dead) |
| `C-202600010-N00085` | samostatny_nalez | pr=C, stav=4 (public-archived SN); files `cafc9a90-3b8b-40f8-ace6-5791b0b6ffd2` (soub-653685), `e5115f1a-72ce-4eaa-a2d9-f52544aaa370` (soub-653686) | landing, OAI, handle, file, backend (2026-09-15: anon/B/C 200 with masked chranene ✓D06 fixed; file anon 200 ✓ / B 403 ✗D09) |
| `C-202009779-N00022` | samostatny_nalez | pr=D, stav=1, maintainer-provided (2026-09-15) creator-org clause probe — both arms match: `predano_organizace` ORG-000091, projekt `C-202009779` org ORG-000091; created SN01 by U-001975 | landing, OAI (2026-09-15: anon 401/403 body; C landing 200 ✓ org clause, C OAI 403 body ✗D08; D 200 full) |
| `C-202211308-N00230` | samostatny_nalez | pr=D, stav=1, maintainer-provided (2026-09-15) creator-org clause probe — projekt arm only: `predano_organizace` ORG-000099 (Archaia Brno, mismatch), projekt `C-202211308` org ORG-000091 (match); created SN01 by U-004219 | landing, OAI (2026-09-15: anon 401/403 body; C landing 200 ✓ projekt-arm org clause, C OAI 403 body ✗D08; D 200 full) |
...
## Current verification (2026-09-15 regression pass on the #370 follow-up deploy; production posture probes)
...
**Provenance note:** the 2026-09-15 pass re-verified the D06/D08/D09 open cells with fresh role sessions (B/C) and anonymous/OAI probes on the new test deploy, the D07/D10/D11/D12/D14 fix holdouts, the `uzivatel` own-record clause (previously harvest-lagged), and the production posture (fedora read-action removal, D07, D12, D14, `/thumb-large` absence); cells carrying only earlier dates were observed on a previous deploy of the same fix-wave family and were not re-probed this pass.

### 2026-09-15 pass summary — defect outcomes

| Defect | Outcome on test | Production posture |
| --- | --- | --- |
| D06 | **fixed**: anonymous and B now receive public stav=4 SNs on landing, handle, and OAI (record served, chranene masked per element rules) — the A/B `pristupnost <= userPr` divergence from #237 is gone | production not re-probed for this row (D14/D07 evidence shows the fix wave is not deployed there) |
| D07 | **still fixed**: restricted VBs stay countable (8 elevated) but the anonymous search response carries no geometry fields | **open — deferred** (operator 2026-09-15: resolved by the upcoming production release): `entity=vyskovy_bod&q=-stav:3` → numFound 11,530 restricted-children docs; first doc serves full unsuffixed geometry (`ADB-BERO01-000001-V0001`, EPSG:5514 POINT Z) |
| D08 | **narrowed — still open**: public stav=4 SNs are now served to anonymous/B on OAI (fixed with D06); the **B-owner and C-org clauses are both dead on OAI** (landing/entities path serves both: owner 200, org-match 200, projekt-arm org-match 200 — OAI returns the 403 body in every case) | — |
| D09 | **still open** (re-verified): B gets 403 on a stav=4 SN file that anonymous fetches (200) | — |
| D10 | **still fixed**: variant matrix conforms (orig 403, `/thumb` 200, `/thumb/page/1` 403, `/thumb-large` 403, `/paradata` 403 — anonymous) | endpoint absent (404) |
| D11 | **still fixed**: komponenta search serves no chranene block to anonymous | production now **indexes** komponenta docs (250,496) but its `entity=komponenta` query path errors (old searcher) — no leak path exercised; the komponenta scenario owns the record-level surface |
| D12 | **still fixed**: 38 restricted projekts searchable, no chranene block in anonymous docs | **still fixed**: `M-202400606` (pr=C) serves no chranene block anonymously |
| D13 | read actions (`GET_ID`, `REQUEST`) removed — 500 no-enum from inside the gate-passed branch; the raw-read leak is closed | **fixed by hotfix**: production removed `GET_ID` and `REQUEST` as well (500 no-enum, Tomcat 11.0.15); the network-trusting gate posture itself remains (maintainer decision item; remaining actions unprobed — mutating) |
| D14 | **still fixed**: `/pdf` 401 anonymous on a restricted dokument (soubor-id shape) | **open — live leak, deferred** (operator 2026-09-15: resolved by the upcoming production release): `/pdf` serves **real page JPEGs of restricted dokumenty to anonymous clients** (warm cache; two samples: 115,946 B and 55,523 B) while the gated `/thumb/page/1` returns 403 |

**Issue #370 checklist mapping (2026-09-15):**

1. *Non-archived records visible after login (landing only)* — ✓ re-verified anonymous side: `C-202204147A` 401, `C-TX-193001369` 401 (B/C/D 200 verified 2026-09-11).
2. *Same conditions as the API (#237)* — ✓ at record level on the test build: SN A/B rows now follow `stav=4` (D06 fixed); remaining divergences are D08 (OAI B-owner clause) and D09 (file-gate B branch) — both B-row clauses, no anonymous exposure.
3. *404/410/401/403 distinction* — ✓ re-verified anonymous: fabricated ident 404; deleted SN `C-202204159-N00003` 410; unarchived akce/dokument 401; (403 cells carried from the fix-wave passes).
4. *Correct return codes* — ✓ on the test build across surfaces; production residuals: `/pdf` 200 with real content instead of 401 (D14), VB geometry served in search (D07).
...
| SN pr=C stav=4 (`C-202600010-N00085`) | 200 ✓ (masked; 2026-09-15 — fixed) | 200 ✓ (masked; 2026-09-15 — fixed) | 200 ✓ (2026-09-11) | 200 ✓ |
...
| SN pr=C stav=1 B-owned | 403 body ✓ | 403 body ✗D08 (owner clause dead; 2026-09-15 — re-verified) | 403 body ✓ (org mismatch — rule-conformant; 2026-09-11) | 200 ✓ (full) |
...
| SN pr=D stav=1 org-match (`C-202009779-N00022`, both arms match) | 403 body ✓ | — | 403 body ✗D08 (org clause dead on OAI; landing 200 — 2026-09-15) | 200 ✓ (full; 2026-09-15) |
| SN pr=D stav=1 projekt-org match (`C-202211308-N00230`, predano arm mismatches) | 403 body ✓ | — | 403 body ✗D08 (projekt-arm org clause dead on OAI; landing 200 — 2026-09-15) | 200 ✓ (full; 2026-09-15) |
...
Notes: legacy `/api/search/id` **action removed** in the 2026-09-15 builds (500 no-enum, no data served); `entity=dokumentacni_jednotka`/`dokument_cast` are pre-existing stubs; `D07` on test — anonymous `entity=vyskovy_bod&q=-stav:3` serves no geometry fields (re-verified 2026-09-15); production still returns 11,530 restricted-children docs with unsuffixed geometry (re-verified 2026-09-15). Search-side stats note (2026-09-15): the restricted-projekt sweep's `lat_A` stats field counts 5 populated docs, but no served field list projects coordinates or katastr for them — the aggregate min/max is the only anonymous-visible signal (coarse, country-scale); likely pian-derived, unconfirmed.
...
| --- | --- | --- | --- | --- |
| dokument pr=C stav=3 (`M-TX-202300441`, `M-TX-202100125`) — orig | 403 ✓ (2026-09-15) | 403 ✓ | 200 ✓ | — |
| └ `/thumb` | 200 ✓ (documented ungated; 2026-09-15) | — | — | — |
| └ `/thumb/page/1` | 403 ✓ (2026-09-15) | — | — | — |
| └ `/thumb-large` | 403 ✓ — **still fixed 2026-09-15** (D10) | — | — | — |
| └ `/paradata` | 403 ✓ (2026-09-15) | — | — | — |
| SN pr=C stav=4 (`C-202600010-N00085`) — orig | 200 ✓ (A rule; 2026-09-15) | 403 ✗D09 (2026-09-15 — still open) | 200 ✓ (2026-09-11) | 200 ✓ |
...
| `/pdf?id=<restricted dokument soubor id>&page=1` | ✓ 401 — **still fixed 2026-09-15** (D14: `ImageAccess.isAllowed(…, true)` gate) | — | — | — |
| `/pdf?id=<SN soubor id>&page=1` | ✓ gate passes per #85; cold cache serves the 7,525 B placeholder only — no content leak | — | — | — |
| production `/pdf?id=<restricted dokument>&page=1` | ✗ 200 with **real page JPEG content** (2026-09-15: 115,946 B for `soub-340129`, 55,523 B for `soub-847150` — both restricted dokumenty; the page cache has warmed, D14 escalated to a live leak) | — | — | — |
...
| --- | --- | --- | --- | --- |
| test `/fedora/get_id?id=<restricted projekt>` | n.a. — action removed (`No enum constant …GET_ID` 500; re-observed 2026-09-15 — the request passes the network-trusting gate from this position) | — | — | — |
...
| D06 | High | SN record level | **fixed on test** (2026-09-15: anon/B 200 with chranene masked on the public stav=4 SN, landing and OAI) | the A/B branches' `pristupnost <= userPr` conditions contradicted #237's `stav=4` rule |
| D07 | Medium-High | search API (vyskovy_bod) | **fixed on test; open on production — deferred** (operator 2026-09-15: resolved by the upcoming production release) | restricted VB geometry anonymously searchable |
| D08 | High | OAI-PMH (SN) | **open — narrowed** (2026-09-15: anon/B now 200 masked on public SNs via the D06 fix; B-owner and C-org clauses both work on the landing/entities path but are dead on OAI) | SN OAI owner/organisation clauses read fields the oai doc does not resolve |
| D09 | Medium | file API | **open** (re-verified unchanged 2026-09-15) | authenticated B denied a stav=4 file anonymous fetches |
| D10 | High | file API `/thumb-large` | **fixed on test** (2026-09-15: 403 anonymous re-verified); production never had the endpoint (404) | undocumented endpoint bypassed all permission checks |
| D11 | High | backend surfaces (komponenta) | **fixed on test** (2026-09-15: re-verified) | komponenta docs served the SN chranene block unmasked |
| D12 | High | search API (projekt) | **fixed on test and production** (2026-09-15: `M-202400606` masked on prod re-verified) | anonymous search served full `projekt_chranene_udaje` for restricted projekts |
| D13 | High | `/fedora/*` | **fixed — read actions removed by hotfix on production and test** (2026-09-15); residual = the network-trusting gate design and unprobed mutating actions | unauthenticated raw-Fedora access from the local network/VPN |
| D14 | High | `/pdf` | **fixed on test** (2026-09-15: 401 anonymous re-verified); **open on production — live leak, deferred** (operator 2026-09-15: resolved by the upcoming production release) | PDF page JPEGs served with no permission gate |
...
- **D08 (High, narrowed 2026-09-15):** both SN record clauses now demonstrably work on the landing/entities path and fail only on OAI. The B-owner clause still returns the 403 body on OAI (`C-202600009-N00014` as B-owner: landing 200, OAI 403 — re-verified 2026-09-15). The C creator-organisation clause was exercised 2026-09-15 with the maintainer-provided records: `C-202009779-N00022` (pr=D, stav=1, both arms match: `predano_organizace` ORG-000091 and projekt `C-202009779` org ORG-000091) and `C-202211308-N00230` (pr=D, stav=1, projekt arm only: `predano_organizace` ORG-000099 [Archaia Brno] mismatches, projekt `C-202211308` org ORG-000091 matches) — C (U-004496, ORG-000091) gets **200 on the landing page for both** (each org arm verified) but the **403 body on OAI GetRecord for both**. Anonymous gets 401 on landing and the 403 body on OAI (rule-conformant). So the clause logic itself is sound on the entities path (the earlier `typ_zmeny='D01'` vs `SN01` source concern does not break it there); what remains is the oai-document shape not resolving the fields the owner/organisation clauses read — or the clauses failing on that path. The anon/B public-SN arm is resolved by the D06 fix. C-org behaviour on the landing surface is rule-conformant on every probed combination (org-match 200 both arms; earlier org-mismatch record 403). The uzivatel own-record clause is now verified working (B sees own `U-004495` record on OAI; C-other and anon 403) — the earlier oai-core harvest lag resolved.
- **D09 (Medium, re-verified open 2026-09-15):** unchanged shape — B gets 403 on the stav=4 SN file (`C-202600010-N00085` orig, file `cafc9a90-3b8b-40f8-ace6-5791b0b6ffd2`) that anonymous fetches (200), while the SN01 owner clause works on the file surface (B-owner 200 on `C-202600009-N00014`). The logged-in branch of the file gate still applies a pristupnost comparison without the stav=4 exemption the anonymous branch honors. Fix direction: apply the per-role file rules uniformly regardless of authentication.
...
- **D12 (High, fixed on test and production — re-verified 2026-09-15):** anonymous search no longer serves `projekt_chranene_udaje` for restricted projekts — on test, 38 restricted projekts are searchable with the block absent; on production, `M-202400606` (pr=C) still serves no chranene block anonymously (re-verified 2026-09-15). The unsuffixed projection in `ProjektSearcher`'s field list noted in the 2026-09-10 audit no longer produces a leak on either deployment.
- **D13 (High, fixed by hotfix 2026-09-15 — read actions removed):** production `/fedora/get_id?id=C-202210658` and `/fedora/request?url=…` now return the 500 no-enum error (`FedoraServlet.Actions.GET_ID`/`REQUEST` no longer exist, Tomcat 11.0.15) — the raw-XML read paths that served full `chranene_udaje` unauthenticated from the trusted position are gone on both deployments. Residuals, for the maintainers' design decision: the servlet's network-trusting gate itself is unchanged (the no-enum 500 is thrown inside the gate-passed branch — this VPN/local-network position still passes without login), and the remaining actions are unprobed mutating index-management ones (INDEX_FULL, INDEX_UPDATE, REINDEX_FILTER, …) — probing them would mutate the index and was out of scope. Fix direction for the residual: require authentication even from trusted networks, or narrow `allowedIP` and document the exception.
